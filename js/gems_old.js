...
class Gems {
  ...
  filter(setCols, sortBy = "price", desc = false) {
    ...
    return new Proxy({
      ...
      offset: 12, 
      page: 0, 
      get items() { return this._indeces.length },
      get pages() { return Math.ceil(this._indeces.length / this.offset) }
    }, {
      get(target, prop) {
        const i = Number(prop);
        ...
        target.page = i;
        if (target.#cache[i]) return target.#cache[i];
        const start = i * target.offset;
        const p = target._indeces.slice(start, start + target.offset).map(idx => target._gems[idx]);
        target.#cache[i] = p;
        return p;
      }
    });
